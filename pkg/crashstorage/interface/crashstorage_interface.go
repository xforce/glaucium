...
type CrashStorage interface {
	...
	NewCrashes(callback func(string))
...
func (p *CrashStorageBase) NewCrashes(callback func(string))                          {}
...
func NewFileDumpsMapping() FileDumpsMapping {
	return FileDumpsMapping{M: make(map[string]interface{})}
}

func (p *MemoryDumpsMapping) AsFileDumpsMapping(crashId string, tempPath string, dumpFileSuffix string) DumpsMapping {
	mapping := NewFileDumpsMapping()

	for k, v := range p.M {
		dumpPathName := path.Join(tempPath, fmt.Sprint(crashId, ".", k, ".TEMPORARY", dumpFileSuffix))
